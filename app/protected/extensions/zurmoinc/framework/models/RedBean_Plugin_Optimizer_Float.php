<?php
    ...
    /**
     * A Float only version of RedBean's existing RedBean_Plugin_Optimizer_Datetime.
     ...
     * so there is no point in repeating it even though the float optimizer should not
     ...
     */
    class RedBean_Plugin_Optimizer_Float extends RedBean_Plugin_Optimizer_Datetime
    {
        public function optimize($columnMaxLength = 0, $columnPrecision = 0)
        {       
            ...
            try
            {
                ...
                if (array_key_exists($this->column, $columnNamesToTypes))
                {
                    ...
                    if ($columnType != 'float')
                    {
                        $this->adapter->exec("alter table {$this->table} change {$this->column} {$this->column} 
                                                                         float( ". $columnMaxLength ." , 
                                                                         ". $columnPrecision .")");
                    }
                }
                else
                {
                    $this->adapter->exec("alter table {$this->table} add {$this->column} float( ". $columnMaxLength ." , 
                                                                                                ". $columnPrecision .")");
                }
            }
